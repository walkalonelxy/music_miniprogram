...
Page({
  ...
  async getVideoGroupListData(){
    let videoGroupListData = await request({ url: '/video/group/list' });
    this.setData({
  ...
  async getVideoList(navId){
    ...
    let videoListData = await request({ url: '/video/group', data: {id: navId} });
    // 关闭消息提示框
    wx.hideLoading();
   ...
   * 用户点击右上角分享
   */
  onShareAppMessage: function (current) {
    console.log('c',current);
    // 如果当前是通过点击button触发的转发
    if (current.from === 'button') {
      const id = current.target.id;
      const currentVideo = this.data.videoList.find(item => id === item.data.vid).data;
      return {
